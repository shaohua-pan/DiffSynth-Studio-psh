...
# state_dict = load_state_dict('/root/paddlejob/workspace/env_run/panshaohua/code/DiffSynth-Studio-psh/ckpts/Wan2.1-Fun-1.3B-InP_full_fix/step-50.safetensors')
...
if total_latent_sections > 4:
    latent_paddings = [0] + [2] * (total_latent_sections - 3) + [1, 0]
post_image, post_image2x, post_image4x = None, None, None
# post_image = [Image.fromarray(reader.get_data(80)), Image.fromarray(reader.get_data(81)), Image.fromarray(reader.get_data(82)), Image.fromarray(reader.get_data(83))]
for idx, latent_padding in enumerate(latent_paddings):
    ...
    # if is_last_section:
        # save_video(videos, f"output/aimier_1.3B_framepack4_all.mp4", fps=15, quality=5)
        # break
    video = pipe(
    ...
    save_video(post_image, f"output/aimier_1.3B_framepack4_preview{idx}.mp4", fps=15, quality=5)
    videos = video[-4:] + videos
    save_video(video, f"output/aimier_1.3B_framepack4_{idx}.mp4", fps=15, quality=5)
